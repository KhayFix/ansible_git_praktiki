...
* [Intro](#intro)  
  ...
  * [Структура](#структура)
  * [Переосмысление Ansible](#переосмысление-ansible)  
...
### Переосмысление Ansible
* Плейбуки - **ЧТО** (объекты)
* Роли - **КАК** (действия)
* Инвентори - **КУДА** (окружения)
...
* зашифрованными могут быть как файлы целиком, так и отдельные переменные внутри простого файла.  
    Несмотря на то, что кажется удобным и читабельным хранить зашифрованные значения прямо внутри _group_vars_,  
    на самом деле первая же вынужденная смена паролей от шифрованных переменных покажет,  
...
Первая сложность при создании автотестов для Ansible-проекта - это поиск соответствующего инструмента.  
...
* Molecule рассчитана в первую очередь на тестирование ролей, поэтому для упрощения покрытия "кода"  
    тестами стоит для начала перейти к модели, в которой все изменения вынесены в отдельные роли.  

* Основным драйвером в Molecule является Docker, а тесты выполняются в контейнерах.  
    Если раннеры вашего CI/CD уже запущены в контейнерах, то придётся задуматься о _Docker-in-Docker_.  
    А если планируете тестировать сервисы вне контейнеров -- настроить работу _Systemd_ в контейнере.  

Molecule обеспечивает роли полный набор тестов, включая линтеры, тестовый деплой,  
проверку на идемпотентность, внешние unit-тесты с использованием Python-based фреймворков.  

Тестирование роли выглядит примерно так:

* Линтеры (yamllint, ansible-lint, flake8).  
    Проверяют соответствие кода общепринятым best practices и специфичным для конкретных языков code styles.
* Тестовый деплой роли в подготовленный контейнер.  
    Выявляет проблемы в функционале роли.  
    Этот шаг имеет смысл запускать с дефолтными переменными, чтобы проверить изоляцию роли.
* Второй деплой роли в тот же контейнер.  
    Проверяет идемпотентность роли.
* Внешние тесты (по умолчанию pytest+testinfra).  
    Позволяют написать unit-тесты для проверки изменений, вносимых ролью.
...
Примерная последовательность тестов для монорепозитория:  

* `molecule lint` - для каждой роли, на каждый коммит  

    *Локальный пример:*  

    `cd role/users && molecule lint`  

    *Локальный пример для всех ролей сразу:*  
...
* `molecule test` - для каждой роли, только при открытом Merge Request в master  

    *Локальный пример для одной роли:*  

    `cd role/users && molecule test`  
...
* `yaml-lint` - на все YaML-файлы за пределами ролей, на каждый коммит  

    *Локальный пример:*  

    ```bash
    find . -type f -name '*.yml' -not -regex '^./roles/.*'|parallel -P 4 -I yaml_file unbuffer yamllint yaml_file
    ```

* `ansible-lint` на каждый плэйбук, на каждый коммит  

    *Локальный пример для всех плэйбуков:*  

    ```bash
    find playbooks -maxdepth 2 -type f -name '*.yml' | parallel -P 4 -I playbook unbuffer ansible-lint -p playbook
    ```

* `ansible all --list-hosts` - на каждый из инвентори, на каждый коммит

    *Локальный пример:*  

    ```bash
    ANSIBLE_INVENTORY_ANY_UNPARSED_IS_FAILED=true ansible -i inventory_path all --list-hosts
    ```
